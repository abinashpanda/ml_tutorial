...
def plot_(clf, feature_generator, order, range_X_min,
          range_X_max, color, label_):
...
if __name__ == '__main__':
    ...
    plot_(clf, feature_generator, order, np.min(X), np.max(X),
          'm', 'Ridge Regression')
    plot_(clf_biased, feature_generator, order, np.min(X), np.max(X),
          'c', 'Linear Regression without Regularization')

    plt.plot(np.arange(np.min(X), np.max(X), 0.01), np.sin(np.arange(np.min(X),
             np.max(X), 0.01)), 'g', label='Target Function')
    plt.legend(loc='best')
    plt.show()
